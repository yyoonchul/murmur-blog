...
export default function Settings() {
  ...
  useEffect(() => {
    loadSettings()
      .then((data) => applyStateFromData(data))
      .catch((err: Error) => setMessage({ type: "error", text: err?.message || "Failed to load settings" }))
      .finally(() => setLoading(false));

    getPersonas()
      .then((data) => setPersonasData(data))
      .catch((err: Error) => setPersonasMessage({ type: "error", text: err?.message || "Failed to load personas" }))
  ...
  const handleAddCustomModel = (provider: ProviderType, e: React.FormEvent) => {
    ...
    postCustomModel({
      ...
      .then((data) => {
        setState((prev) => ({
          ...
          allProviderModels: {
            ...
            [provider]: (data.availableModels as ModelOption[] | undefined) ?? prev.allProviderModels[provider],
          },
          availableModels:
            provider === prev.provider
              ? ((data.availableModels as ModelOption[] | undefined) ?? prev.availableModels)
      ...
      .catch((err: Error) => setMessage({ type: "error", text: err?.message || "Failed to add" }))
      .finally(() => setSaving(false));
  };
  ...
  const handleDeleteCustomModel = (provider: ProviderType, modelId: string) => {
    ...
    deleteCustomModel(provider, modelId)
      ...
      .catch((err: Error) => setMessage({ type: "error", text: err?.message || "Failed to delete" }))
      .finally(() => setSaving(false));
  };
  ...
  const loadLibrary = () => {
    ...
    getPersonaLibrary()
      .then((data) => setLibraryPersonas((data.presets || []) as LibraryPersona[]))
      .catch((err: Error) => setPersonasMessage({ type: "error", text: err?.message || "Failed to load library" }))
      .finally(() => setLibraryLoading(false));
  };

  const handleAddPersona = (personaId: string) => {
    setPersonasSaving(true);
    postAddPersona(personaId)
      .then((data) => {
  ...
  return (
    <div className="animate-fade-in">
                      ...
                      <button
                        ...
                        onClick={() => {
                          if (editingLibraryPersona.isActive) {
                            ...
                            setPersonasSaving(true);
                            putLibraryPersona(editingLibraryData.id, {
                              name: editingLibraryData.name,
                              role: editingLibraryData.role,
                              emoji: editingLibraryData.emoji,
                              color: editingLibraryData.color,
      ...
      {activeTab === "data" && (
      <section>
        <h2 className="font-display text-lg font-semibold mb-4">Your data</h2>
        <p className="text-sm text-secondary mb-4">
          Posts, comments, and persona settings are stored in your Supabase project and scoped to your account.
          LLM API keys you add in Providers are encrypted on the server before being stored.
        </p>
      </section>
      )}
    </div>
